"""
Implements an GatedRecurrentUnit RNN
...
from blocks.roles import WEIGHT, BIAS, PARAMETER
...
gru = GatedRecurrent(dim=n_h, activation=Tanh())
linear2 = Linear(input_dim = n_h, output_dim = n_y, name="output_layer")

transform = linear.apply(x)
predict = Sigmoid().apply(linear2.apply(gru.apply(transform,update_inputs=transform,reset_inputs=transform)))

# Cost function
cost = SquaredError().apply(predict,target)

# Initialization
for brick in (gru, linear, linear2):
